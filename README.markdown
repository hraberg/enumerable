...
* `lambda.weaving.annotation.newlambda` - an annotation with target method used to trigger lambda macro expansion. Not needed at runtime.
* `lambda.weaving.annotation.lambdaparameter` - an annotation with target field used to mark static fields as placeholders in lambdas. Not needed at runtime.
* `lambda.weaving.annotation.lambdalocal` - an annotation that whith target field or parameter that is used to add runtime meta data to created lambdas. If empty, no meta data will be added. and the annotation is not needed at runtime.
